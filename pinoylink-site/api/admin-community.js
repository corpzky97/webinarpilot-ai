...
const CLAIMS='https://api.3minapi.com/api/v1/data/hkhjcuyt7fctp8kwax815';
...
function cfg(type){if(type==='lost_found')return {url:LOST_FOUND,key:process.env.THREEMIN_LOST_FOUND_ADMIN_KEY};if(type==='memorial')return {url:MEMORIALS,key:process.env.THREEMIN_MEMORIALS_ADMIN_KEY};if(type==='claim')return {url:CLAIMS,key:process.env.THREEMIN_LOST_FOUND_CLAIMS_ADMIN_KEY};return null}
...
function mutate(type,row,action,reason=''){
  ...
  if(type==='lost_found'){
    ...
    if(action==='needs_more_info'){row.review_status='needs_more_info';row.reviewed_at=t}
    if(action==='possible_match'){row.review_status='possible_match';row.reviewed_at=t}
    if(action==='approve_handoff'){row.review_status='approved_contact_handoff';row.reviewed_at=t}
    if(action==='reject'){row.review_status='rejected';row.reviewed_at=t;row.status='archived';row.archived_at=t}
    if(action==='archive'){row.review_status='closed';row.reviewed_at=t;row.status='archived';row.archived_at=t}
  }
  return row
}
...
export default async function handler(req,res){
  ...
  if(req.method==='PUT'){
    ...
    let allowed;if(type==='lost_found')allowed=new Set(['approve','needs_changes','reject','matched','returned','archive']);else if(type==='memorial')allowed=new Set(['verify','publish','needs_changes','reject','archive']);else allowed=new Set(['needs_more_info','possible_match','approve_handoff','reject','archive']);if(!allowed.has(action))return res.status(400).json({error:'Unsupported action'});
