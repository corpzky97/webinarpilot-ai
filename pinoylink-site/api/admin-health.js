...
export default async function handler(req,res){
  ...
  const optional=new Set(['THREEMIN_AD_BOOKINGS_WRITE_KEY','THREEMIN_PAYMENT_EVENTS_WRITE_KEY','STRIPE_WEBHOOK_SECRET','THREEMIN_DIRECTORY_SUBMIT_WRITE_KEY','THREEMIN_DIRECTORY_ADMIN_KEY','THREEMIN_ANALYTICS_WRITE_KEY','THREEMIN_ANALYTICS_READ_KEY','THREEMIN_NEWSROOM_WRITE_KEY']);
  const config_ok=Object.entries(env).filter(([k])=>!optional.has(k)).every(([,v])=>v);
